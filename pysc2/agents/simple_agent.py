...
class SimpleAgent(base_agent.BaseAgent):
    ...
    def step(self, timestep):
        ...
        logging.info("judge need supply depot")
        if self.need_supply_depot(obs):
            self.func_queue.append(ActionCollection.move_camera_for_building_supply_depot)
            self.func_queue.append(ActionCollection.build_supply_depot)
            return ActionCollection.select_scv(obs)

        if self.need_barracks(obs):
            self.func_queue.append(ActionCollection.move_camera_for_building_barracks)
            self.func_queue.append(ActionCollection.build_barracks)
            return ActionCollection.select_scv(obs)

        if self.need_factory(obs):
            return ActionCollection.build_factory(obs)
